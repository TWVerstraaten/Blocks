...
namespace view {
    namespace widget {
        LineEditBox::LineEditBox(int x, int y, Uint32 w, Uint32 h, const Assets* assetHandler, std::string title)
            : RectWidget({x, y, static_cast<int>(w), static_cast<int>(h)}), m_assetHandler(assetHandler), m_title(std::move(title)) {
        }
        ...
        void LineEditBox::setWidth(int width) {
            m_rect.w = width;
        }

        void LineEditBox::setHeight(int height) {
            m_rect.h = height;
        }

        void LineEditBox::setX(int x) {
            m_rect.x = x;
        }

        void LineEditBox::setY(int y) {
            m_rect.y = y;
        }
