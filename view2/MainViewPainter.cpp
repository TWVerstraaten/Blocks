...
view2::MainViewPainter::MainViewPainter(const view2::MainView* mainView) : m_mainView(mainView) {
    const int     id     = QFontDatabase::addApplicationFont("assets/UbuntuMono-Italic.ttf");
    const QString family = QFontDatabase::applicationFontFamilies(id).at(0);
    m_font               = QFont(family, 12);
}

void view2::MainViewPainter::paint(QPainter& painter, QPaintEvent* event) {
...
std::unique_ptr<QPixmap> view2::MainViewPainter::connectedPixmap(const model::GridXYSet& blocks, const QColor& color) const {
...
void view2::MainViewPainter::drawConnected(const model::GridXYSet& blocks, const QColor& color, QPainter& painter) const {
...
void view2::MainViewPainter::drawCluster(const model::Cluster& cluster, QPainter& painter) {
